...
class POSTFuncionMethod extends ACRUDControllerMethod {
    ...
    {
        $this->httpCode = 400;
        if(isset($params["id_sala"],$params["id_pelicula"],$params["id_formato"],$params["id_idioma"],$params["id_subtitulos"], $params["hora"], $params["dia"], $params["precio_adulto"], $params["precio_adol"], $params["precio_nino"])){
                ...
                if($idSala > 0 && $idPelicula > 0 && $idFormato > 0 && $idIdioma > 0 && $precioAdulto > 0 && $precioAdol > 0 && $precioNino > 0){
                    ...
                    if($sala instanceof Sala){
                        ...
                        if($pelicula instanceof Pelicula){
                            ...
                            if(Funcion::isHorarioDisponible($sala->getId(), $params["hora"], $params["dia"], $pelicula->getDuracion())){
