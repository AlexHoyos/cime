...
class UsuarioDB extends ADBModel {
    ...
    public static function transformRow($row): UsuarioDB|null {
            if(sizeof($row) > 0)
                return new UsuarioDB($row->id, $row->nombre, $row->apellido, $row->correo, $row->telefono,
    ...
    public static function getAll():DBPagination{
        return UsuarioDB::_getRows(Self::class);     
    }
    
    static public function getById($id): UsuarioDB|null {
        $id = intval($id);
